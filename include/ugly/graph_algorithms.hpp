...
#include "../../src/libugly/hash_specializations.hpp"
...
namespace ugly {
  ...
  namespace graphalgorithms {
    ...
    template<class... Ts>
      std::unordered_map<std::pair<int,int>,double> maxMinimumDistanceBetweenEveryVertex( Graph<Ts...>& graph){

        bool graph_directional = graph.directional();

        std::unordered_map<std::pair<int,int>,double> maxMinDistanceOfGraphVertices;
        auto vertices = graph.getVertices();
