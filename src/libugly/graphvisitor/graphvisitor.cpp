...
bool GraphVisitor::edgeCanBeAdded(weak_ptr<Edge> edge_ptr) {
  if (find(edges_to_explore_.begin(), edges_to_explore_.end(), edge_ptr) !=
...
void GraphVisitor::exploreEdge(weak_ptr<Edge> edge_ptr) {
  if (potentialEdgeKnown_(edge_ptr) == false) {
    throw invalid_argument(
        "Cannot explore an edge that has not yet been "
  ...
  if (explored_edges_.count(edge_ptr)) {
    throw invalid_argument(
        "Cannot explore edge that has already been "
        "explored.");
  }
...
void GraphVisitor::exploreEdge_(weak_ptr<Edge>) {
  throw runtime_error(
      "Cannot call exploreEdge_ from base class must define it"
...
void GraphVisitor::addEdge(weak_ptr<Edge> edge_ptr) {
  ...
  if (!starting_vertex_set_) {
    throw runtime_error(
        "Cannot add edges before specifying the starting "
        "vertex.");
  }
  if (!canAddEdge_(edge_ptr)) {
    throw runtime_error(
        "Cannot add edge as non of the vertices in the edge "
  ...
  if (!edgeTypeAllowed_(edge_ptr)) {
    throw runtime_error(
        "Cannot add edge of this type to the visitor it is "
        "not allowed.");
  }
  addEdge_(edge_ptr);
...
void GraphVisitor::addEdges(vector<weak_ptr<Edge>>) {
  throw runtime_error(
      "Cannot call addEdges from base class must define it in"
      " the derived class.");
}
...
int GraphVisitor::getUnexploredVertex(weak_ptr<Edge> edge_ptr) const {
  if (verticesHaveBeenExplored(edge_ptr)) {
    throw invalid_argument(
        "Cannot find unexplored vertex as both vertices "
        "have been explored.");
  }
  if (auto edge = edge_ptr.lock()) {
    if (explored_vertices_.count(edge->getVertex1())) {
  ...
  throw invalid_argument(
      "Neither vertices have been explored so you cannot "
...
void GraphVisitor::addEdge_(weak_ptr<Edge>) {
  throw runtime_error(
      "Cannot call addEdge from base class must define it in "
      "the derived class.");
}
...
weak_ptr<Edge> GraphVisitor::getNextEdge_() {
  throw runtime_error(
      "Cannot call getNextEdge from base class must "
