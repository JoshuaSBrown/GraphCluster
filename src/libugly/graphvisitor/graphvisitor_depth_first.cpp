...
double GraphVisitorDepthFirst::getDistanceOfVertex(int vertex) {
  ...
  throw invalid_argument(
      "Cannot get distance to vertex as the vertex has not"
      " yet been explored.");
}
...
void GraphVisitorDepthFirst::addEdge_(weak_ptr<Edge>) { return; }
...
weak_ptr<Edge> GraphVisitorDepthFirst::getEdgeShortestDistance_() {
  ...
  for (auto edge_it = edges_to_explore_.begin();
       edge_it != edges_to_explore_.end(); ++edge_it) {
  ...
  if (edge_uninitialized) {
    throw runtime_error(
        "Cannot grab edge shortest distance away because "
...
double GraphVisitorDepthFirst::getDistanceFromStartingVertexToEdge_(
  ...
  if (verticesHaveBeenExplored(edge_ptr)) {
    ...
    if (!edgeTypeAllowed_(edge_ptr)) {
      throw invalid_argument(
          "Edge type is not allowed in the depth first graph"
          " visitor.");
    }
  }
