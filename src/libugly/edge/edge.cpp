...
int Edge::getOtherVertex(int vertex) const {
  ...
  throw runtime_error(
      "Unable to determine the other vertex as the provided "
...
int Edge::getMin() const {
  if (vertex1_ == constants::unassigned || vertex2_ == constants::unassigned) {
    throw runtime_error(
        "Cannot get min vertex as at least one vertex in the "
...
int Edge::getMax() const {
  if (vertex1_ == constants::unassigned || vertex2_ == constants::unassigned) {
    throw runtime_error(
        "Cannot get max vertex as at least one vertex in the "
        "edge has not been assigned.");
  }
...
bool Edge::operator!=(const Edge& edge) const { return !(*this == edge); }
