...
class Edge {
 ...
 public:
  ...
  Edge(int vertex1, int vertex2)
      : edge_directed_(true), vertex1_(vertex1), vertex2_(vertex2) {
    object_type_ = constants::EdgeType::edge;
  }

  Edge(const Edge& edge) {
    std::cout << "Calling Edge copy constructor " << std::endl;
    edge_directed_ = edge.edge_directed_, object_type_ = edge.object_type_;
...
template <class T>
static std::shared_ptr<T> makeEdge(std::weak_ptr<Edge> edge) {

  if (auto edge_ptr = edge.lock()) {
    if (edge_ptr->getEdgeType() == T::getClassType()) {
      return std::shared_ptr<T>(new T(*edge_ptr));
    }
  }
  throw std::invalid_argument("Cannot makeEdge from type");
}
}
